import 'package:aegis/navigator/page_router.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';

enum AegisPushPageType {
  slideToLeft,
  noAnimation,
  opacity,
  transparent,
...
enum AegisStackPageOption { reset, push, pop, replace }

class AegisNavigator extends Navigator {
  ...
  static void popToPage<T extends Object>(BuildContext context,
    ...
    assert(() {
      if (pageType == null && pageId == null) {
        throw FlutterError(
            'The AegisNavigator.popToPage method requires at least one of the parameters: pageType or pageId.');
      }
      return true;
    }());
    // Remove the current focus
    ...
    Navigator.popUntil(context, (Route<dynamic> route) {
      ...
      AegisRouteSettings? settings;
      try {
        settings = route.settings as AegisRouteSettings;
      } catch (_) {}
  ...
  static void _pushPreHandle(BuildContext context) {
    ...
    if (currentSettings is! AegisRouteSettings) return;
  ...
  static Future<T?> pushPage<T extends Object?>(
    ...
    AegisPushPageType type = AegisPushPageType.slideToLeft,
  }) {
    pageName ??= builder(null).runtimeType.toString();
    context ??= navigatorKey.currentContext;
    if (context == null) return Future.value(null);

    final routeSettings = AegisRouteSettings(
      name: pageName,
      pageId: pageId,
      isShortLived: isShortLived,
    );
    ...
    switch (type) {
      case AegisPushPageType.slideToLeft:
        route = SlideLeftToRightRoute<T>(
          fullscreenDialog: fullscreenDialog,
          settings: routeSettings,
          builder: builder,
        );
      case AegisPushPageType.noAnimation:
        route = NoAnimationPageRoute<T>(
          builder: builder,
          settings: routeSettings,
        );
      case AegisPushPageType.opacity:
        route = OpacityAnimationPageRoute<T>(
          builder: builder,
          settings: routeSettings,
        );
      case AegisPushPageType.transparent:
...
class AegisRouteSettings extends RouteSettings {
  const AegisRouteSettings({
    super.name,
    this.pageId,
    bool? isShortLived,
